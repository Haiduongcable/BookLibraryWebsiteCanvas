...
def _load_val_videos_from_file(fp: Optional[str]) -> List[str]:
    vids = []
    if not fp:
        return vids
    p = Path(fp)
    if not p.exists():
        print(f"[WARN] --val_videos_file not found: {fp}")
        return vids
...
def prepare_yolo_dataset(
    ...
    resize_width=None,
    multi_class=False,
    # NEW: explicit val videos
    val_videos_explicit: Optional[List[str]] = None,
):
    """
    ...
    Args:
        ...
        val_videos_explicit: If provided, use these video_ids as validation set (overrides train_split)
    """
    ...
    # --- NEW: explicit val selection overrides random split ---
    if val_videos_explicit is not None and len(val_videos_explicit) > 0:
        chosen_val = list(dict.fromkeys(val_videos_explicit))  # keep order, drop dups
        missing = [v for v in chosen_val if v not in video_annotations]
        if missing:
            print(f"[WARN] These --val_videos are not found in annotations and will be ignored: {missing}")
        val_videos = set(v for v in chosen_val if v in video_annotations)
        train_videos = set(v for v in video_ids if v not in val_videos)
        print(f"[INFO] Using explicit validation set. Ignoring train_split={train_split}.")
    else:
        ...
        while frame_idx <= max_frame_needed:
            ...
            if frame_idx in all_frames_to_extract:
                ...
                if is_annotated:
                    ...
                    with open(output_dir / split / 'labels' / label_name, 'w', encoding='utf-8') as f:
                        f.write('\n'.join(yolo_lines))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_split', type=float, default=0.8,
                        help='Train/val split ratio (default: 0.8). Ignored if --val_videos/--val_videos_file is set.')
    ...
    # NEW: explicit val videos arguments
    parser.add_argument('--val_videos', nargs='*', default=None,
                        help='Explicit list of video_ids for validation split. Overrides --train_split.')
    parser.add_argument('--val_videos_file', type=str, default=None,
                        help='Path to a text file listing video_ids (one per line) to use for validation. Overrides --train_split.')
    ...
    # Merge val video sources (CLI list + file)
    explicit_val = []
    file_vals = _load_val_videos_from_file(args.val_videos_file)
    if file_vals:
        explicit_val.extend(file_vals)
    if args.val_videos:
        explicit_val.extend(args.val_videos)
    ...
    prepare_yolo_dataset(
        ...
        val_videos_explicit=explicit_val if len(explicit_val) > 0 else None,  # NEW
    )
